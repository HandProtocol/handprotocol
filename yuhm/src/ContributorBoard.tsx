...
export function ContributorBoard({ dbConfigured, canWrite, memberName, notify, onAuthRequired }: ContributorBoardProps) {
  ...
  const [credentialType, setCredentialType] = useState('yuhm food rescue training')
  ...
  useEffect(() => {
    ...
    setCredentialType(selected.credential_type ?? 'yuhm food rescue training')
  ...
  if (!dbConfigured) return <section className="panel contributor-empty"><AlertTriangle size={25} /><h2>Volunteer Command needs the yuhm database.</h2><p>Configure Supabase and apply migration 029 before reviewing Contributor readiness.</p></section>
