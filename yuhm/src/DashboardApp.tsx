...
export function DashboardApp() {
  ...
  const [locationPromptOpen, setLocationPromptOpen] = useState(() => new URLSearchParams(window.location.search).get('intent') === 'food' && sessionStorage.getItem('yuhm:location-choice') !== 'complete')
  const locationDialogMotion = useDialogMotion(() => setLocationPromptOpen(false), locationPromptOpen)
  const [menuOpen, setMenuOpen] = useState(false)
  const [sidebarCollapsed, setSidebarCollapsed] = useState(() => localStorage.getItem('yuhm:sidebar-collapsed') === '1')
  const [toast, setToast] = useState('')
  const [authPromptOpen, setAuthPromptOpen] = useState(false)
  const authDialogMotion = useDialogMotion(() => setAuthPromptOpen(false), authPromptOpen)
  ...
  const addSpotDialogMotion = useDialogMotion(() => setAddSpotOpen(false), addSpotOpen)
  const foodHereDialogMotion = useDialogMotion(() => setFoodHereOpen(false), foodHereOpen)
  ...
  const useSimpleMode = () => {
    localStorage.removeItem('yuhm:experience-mode')
  }
  ...
  const toggleSidebar = () => setSidebarCollapsed((current) => { localStorage.setItem('yuhm:sidebar-collapsed', current ? '0' : '1'); return !current })
  ...
  return (
    <div className={`app-shell ${sidebarCollapsed ? 'sidebar-is-collapsed' : ''}`} data-experiment={variant}>
      <aside className={`sidebar ${menuOpen ? 'open' : ''} ${sidebarCollapsed ? 'collapsed' : ''}`}>
        <SidebarHand />
        <div className="brand"><button className="brand-home" type="button" onClick={() => openView('command')} aria-label="Go to yuhm overview"><span className="brand-mark" aria-hidden="true">◯</span><span className="brand-copy"><strong>yu<span>hm</span></strong><small>the om in yum</small></span></button><button className="mobile-close" onClick={() => setMenuOpen(false)} aria-label="Close navigation"><X size={18} /></button></div>
