...
function initialVariant(): MapLabVariant {
  ...
  const favorite = localStorage.getItem('yuhm:map-lab-favorite')
...
export function MapLab({ product = false }: { product?: boolean }) {
  ...
  const [favorite, setFavorite] = useState(() => localStorage.getItem('yuhm:map-lab-favorite'))
  ...
  const setDetentWithHistory = (next: SheetDetent, velocity = 0, fromDrag = false) => {
    if (next === 'full' && !historyEntryRef.current) {
      window.history.pushState({ yuhmSheet: 'full' }, '')
  ...
  return <main className={`map-lab map-lab-${variant}${product ? ' map-lab-product' : ''}`} style={{
    ...
    <motion.section
      ...
      aria-label={sheetMode === 'menu' ? 'yuhm map menu' : sheetMode === 'place' ? 'Selected food place' : 'Food place results'}
    >
      <button
        type="button"
    ...
    {!product && <aside ref={evaluatorRef} className={`map-lab-evaluator ${evaluatorOpen ? 'open' : ''}`} aria-label="Map lab evaluator" aria-hidden={sheetMode === 'menu' ? 'true' : undefined}>
      ...
      {evaluatorOpen && <div className="map-lab-evaluator-panel">
        ...
        <button className="map-lab-favorite" type="button" onClick={() => {
          localStorage.setItem('yuhm:map-lab-favorite', variant)
          setFavorite(variant)
...
function MenuContent({ member, authReady, onClose, onNavigate, product }: { member: User | null; authReady: boolean; onClose: () => void; onNavigate: () => void; product: boolean }) {
  ...
  return <div className="map-lab-menu">
    <div className="map-lab-menu-heading"><div><p className="map-lab-eyebrow">yuhm</p><h1>{t('map.choosePath')}</h1></div><div className="map-lab-menu-tools"><LanguageToggle /><button type="button" onClick={onClose} aria-label={t('common.close')}><X size={19} /></button></div></div>
    ...
    <AppLink className="map-lab-advanced" href="/app/?mode=advanced" onClick={() => localStorage.setItem('yuhm:experience-mode', 'advanced')} onNavigate={onNavigate}><Settings size={18} /><span><b>{t('map.advanced')}</b><small>{t('map.advancedDetail')}</small></span><ArrowUpRight size={16} /></AppLink>
