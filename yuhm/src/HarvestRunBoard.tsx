...
export function HarvestRunBoard({ dbConfigured, canWrite, notify, onAuthRequired }: Props) {
  ...
  const stopDialogMotion = useDialogMotion(() => setShowStop(false), showStop)
  const [runForm, setRunForm] = useState(initialRun)
  const [actionNote, setActionNote] = useState('')
  const [assigneeId, setAssigneeId] = useState('')
  const [scheduleConfirmed, setScheduleConfirmed] = useState(false)
  const [activeStopId, setActiveStopId] = useState<string | null>(null)
  const outcomeDialogMotion = useDialogMotion(() => setActiveStopId(null), activeStopId !== null)
  ...
  if (!dbConfigured) return <section className="panel run-empty"><AlertTriangle size={25} /><h2>Harvest runs need the yuhm database.</h2><p>Configure Supabase and apply migrations through 037 before dispatch testing.</p></section>
