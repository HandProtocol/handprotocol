...
const feedbackQueueKey = 'yuhm:feedback-queue'
const rememberedNameKey = 'yuhm:feedback-name'
...
export function notifyYuhmAccountSignup(email: string) {
  return postFeedback({
    text: `Email: ${email.trim().toLowerCase()}`,
    path: '/app/?mode=login&signup=1',
    title: 'New yuhm account signup',
    name: '',
    tags: ['yuhm', 'account-signup', 'signup'],
    source: 'yuhm Account Signup',
    scroll: 0,
    ua: navigator.userAgent.slice(0, 200),
    vw: window.innerWidth,
    vh: window.innerHeight,
...
export async function sendYuhmFeedback(input: FeedbackInput): Promise<'sent' | 'queued'> {
