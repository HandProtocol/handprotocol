...
export function LoginScreen() {
  ...
  const submitAuth = async (event: React.FormEvent) => {
    ...
    else if (!result.data.session) setError('Your account was created, but yuhm could not log you in. Please try logging in.')
    else {
      if (authMode === 'signup') void notifyYuhmAccountSignup(email)
  ...
  return <div className="login-page"><div className="login-card"><div className="login-card-top"><a className="login-wordmark" href="/">yuhm <small>/THE OM IN YUM ♥</small></a><LanguageToggle /></div>
    ...
    <h1>{isRecovery ? 'Set a new password.' : isReset ? 'Reset your password.' : isUpdates ? 'Get yuhm updates.' : authMode === 'signup' ? t('login.createAccount') : t('login.welcomeBack')}</h1>
