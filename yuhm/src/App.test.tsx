...
describe('yuhm entry points and interaction gates', () => {
  beforeEach(() => {
  ...
  it('switches map lab variants through the URL and stores a favorite locally', async () => {
    ...
    expect(localStorage.getItem('yuhm:map-lab-favorite')).toBe('command-bar')
  ...
  it('moves the shared lab sheet between search, place, and modal menu content', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'yuhm map menu' })).toHaveAttribute('aria-modal', 'true')
    ...
    await waitFor(() => expect(screen.queryByRole('dialog', { name: 'yuhm map menu' })).not.toBeInTheDocument())
  ...
  it('uses the command-bar map as the public mobile food interface', async () => {
    ...
    expect(await screen.findByRole('dialog', { name: 'Get yuhm alerts' })).toBeInTheDocument()
  })
  ...
  it('offers email-only updates without account credentials', async () => {
    ...
    expect(screen.getByRole('heading', { level: 1, name: 'Get yuhm updates.' })).toBeInTheDocument()
  ...
  it('opens the bottom-right alerts panel and joins the updates audience', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Get yuhm alerts' })).toBeInTheDocument()
  ...
  it('replaces the coming-soon card with the live WaterDrop app', () => {
    render(<App />)
    expect(screen.getByRole('heading', { level: 1, name: /Food, shared full circle/i })).toBeInTheDocument()
  ...
  it('keeps a community request draft through the sign-in handoff', async () => {
    ...
    expect(sessionStorage.getItem('yuhm:request-draft')).toContain('Produce for Saturday dinner')
    unmount()
  ...
  it('labels advanced mode and offers a direct return to simple mode', async () => {
    ...
    expect(localStorage.getItem('yuhm:experience-mode')).toBeNull()
  })
  ...
  it('keeps public browsing open but gates FOOD IS HERE behind an account', async () => {
    ...
    expect(screen.getByRole('link', { name: /Email me yuhm updates/i })).toHaveAttribute('href', '/app/?mode=login&updates=1')
  })
  ...
  it('keeps FOOD IS HERE visible on Overview and provides a dedicated workspace', async () => {
    ...
    expect(screen.getByLabelText('Current location')).toHaveTextContent('Directory/yuhm/Food available now')
  })
  ...
  it('returns to the top when switching command-center views', async () => {
    ...
    expect(screen.getByLabelText('Current location')).toHaveTextContent('Directory/yuhm/Rescue operations')
  })
  ...
  it('returns to the overview from the yuhm title', async () => {
    window.history.replaceState({}, '', '/app/?mode=advanced')
    render(<App />)

    await userEvent.click(screen.getByRole('button', { name: /^Rescue operations$/i }))
    await userEvent.click(screen.getByRole('button', { name: 'Go to yuhm overview' }))

    expect(screen.getByRole('heading', { name: 'Local food, coordinated.' })).toBeInTheDocument()
    expect(screen.getByLabelText('Current location')).toHaveTextContent('Directory/yuhm/Overview')
  })
  ...
  it('replaces the rescue placeholder with an honest database and account gate', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /^Rescue operations$/i }))
    expect(screen.getByText(/Rescue operations need the yuhm database/i)).toBeInTheDocument()
  ...
  it('replaces the volunteer placeholder with the private readiness account gate', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /Volunteer Command needs the yuhm database/i })).toBeInTheDocument()
  ...
  it('opens the real harvest run workspace from navigation', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /Harvest runs need the yuhm database/i })).toBeInTheDocument()
  })
  ...
  it('opens the real inventory workspace from navigation', async () => {
    ...
    expect(screen.getByRole('heading', { level: 2, name: /Inventory needs the yuhm database/i })).toBeInTheDocument()
  })
  ...
  it('opens the drop-off log with its privacy boundary and database gate', async () => {
    ...
    expect(screen.getByLabelText('Current location')).toHaveTextContent('Directory/yuhm/Drop-off log')
  })
})
