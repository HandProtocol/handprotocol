...
const ROOT = '/home/koh/Documents/handprotocol/web/project/subculture-audio'
const slug = process.argv[2]
if (!slug) { console.error('need a slug (blueprint, heavyweight, afterdark) or "index"'); process.exit(1) }
const file = slug === 'index' ? `${ROOT}/index.html` : `${ROOT}/${slug}/index.html`
const out = process.argv[3] || '/tmp/sca-shots'
...
async function shoot(name, viewport, fullPage) {
  ...
  // Walk the page so loading="lazy" images are fetched before a full-page capture.
  if (fullPage) {
    await page.evaluate(async () => {
      for (let y = 0; y < document.documentElement.scrollHeight; y += 600) { scrollTo(0, y); await new Promise((r) => setTimeout(r, 120)) }
      scrollTo(0, 0)
    })
