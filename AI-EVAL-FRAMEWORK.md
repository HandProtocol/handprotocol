...
| **Governance** | Is the work accountable to the Companions and Contributors whose work shapes it? |
| **Sovereignty** | Does each Companion group actually own, control, and remain able to leave with their system? |
...
- **Voice / register coherence**: Likert 1–5 from HAND staff blind-rating outputs against unedited samples from the same Companion group. Target: median ≥ 4 by month 9 of the pilot.
...
| **Monthly** | Companion-group-facing summary email: what the agent helped with this month, what it got wrong, what changed. Plus the cross-group contamination check. |
...
- Companion-value: the POC Companion group rates the experience 4+ on both felt-continuity and felt-presence.
...
- Cohort capacity demonstrated at 4+ Companion groups with the same program lead.
...
- Companion-group-reported value sustained at month-12 levels through the production transition.
- Sovereignty: successful closure-simulation drill with the volunteer Companion group, with all gaps documented and addressed before sign-off.
- Decision on weight-publication norms made jointly with the participatory panel; default-no remains the default unless a Companion group has affirmatively chosen otherwise for their adapter.
...
- **Cross-Companion-group knowledge transfer.** Whether and how patterns learned working with one group are made available to another group's agent. Defaulting to *off* during the pilot. Opt-in only, and only by mutual consent if it ever happens.
...
- **Federation across Companion groups.** Whether multiple groups could share certain agent infrastructure (a common harm-reduction knowledge layer, for instance) is a year-three question. Sovereignty defaults remain *separate* until that work is done.
