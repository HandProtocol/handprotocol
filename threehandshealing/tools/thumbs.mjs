...
const ROOT = '/home/koh/Documents/handprotocol/web/threehandshealing'
const OUT = `${ROOT}/styles/_shots`
...
const items = process.argv.slice(2).map(a => { const i = a.indexOf('='); const slug = i < 0 ? a : a.slice(0, i); const url = i < 0 ? undefined : a.slice(i + 1); return { slug, url: url || `file://${ROOT}/styles/${slug}/index.html` } })
...
for (const { slug, url } of items) {
  const d = `/tmp/thh-${slug}-d.png`, m = `/tmp/thh-${slug}-m.png`
