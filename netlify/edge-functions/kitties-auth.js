// Password-only cookie gate for /kitties/* paths.
//
// Set env var in Netlify dashboard:
//   KITTIES_PASSWORD  — required. The shared password.
//
// Flow:
//   1. GET without valid cookie → renders a styled password form.
//   2. POST with correct password → sets HttpOnly cookie + 303 redirects back.
//   3. Subsequent GET with valid cookie → passes through to the static page.
...
export default async (request, context) => {
  ...
  if (!expected) {
    return new Response(
      "Kitties page is misconfigured: KITTIES_PASSWORD env var is not set.",
...
<head>
  ...
  <title>Kitties | HAND Protocol</title>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .gate__brand{display:flex;align-items:center;gap:.6rem;justify-content:center;color:#0C1220;margin-bottom:2rem;font-weight:600;letter-spacing:-0.01em}
    .gate__brand svg{color:#D97706}
    .gate__card{background:#FFFFFF;border:1px solid #E5E7EB;border-radius:16px;padding:2.25rem 2rem;box-shadow:0 10px 40px -10px rgba(15,23,42,0.10), 0 2px 6px rgba(15,23,42,0.04)}
    .gate__eyebrow{display:inline-flex;align-items:center;gap:.5rem;font-size:.7rem;text-transform:uppercase;letter-spacing:.12em;color:#D97706;font-weight:600;margin-bottom:.75rem}
    .gate__eyebrow-dot{width:6px;height:6px;border-radius:99px;background:#D97706}
    .gate__title{font-family:'Source Serif 4','Source Serif Pro',Georgia,serif;font-size:1.65rem;line-height:1.2;letter-spacing:-0.015em;color:#111827;margin-bottom:.5rem}
    ...
    .gate__input{width:100%;padding:.85rem 1rem;border:1px solid #D1D5DB;border-radius:10px;font:inherit;font-size:1rem;color:#111827;background:#FFFFFF;transition:border-color 150ms ease, box-shadow 150ms ease}
    ...
    .gate__error{margin-top:-.25rem;padding:.65rem .85rem;background:#FEF2F2;border:1px solid #FECACA;color:#B91C1C;border-radius:8px;font-size:.85rem}
    .gate__submit{margin-top:.5rem;padding:.9rem 1rem;background:#0C1220;color:#F9FAFB;border:none;border-radius:10px;font:inherit;font-weight:600;font-size:.95rem;cursor:pointer;transition:background 150ms ease, transform 80ms ease}
...
<body>
  <main class="gate">
    ...
    <div class="gate__card">
      <div class="gate__eyebrow"><span class="gate__eyebrow-dot"></span>Private page</div>
      <h1 class="gate__title">Jesuscoins's Kitty World</h1>
      <p class="gate__lede">This page is password protected. Enter the access password to continue.</p>
      ${errorBlock}
      ...
      <p class="gate__foot"><a href="/">← HAND Protocol</a></p>
    </div>
  </main>
</body>
</html>`;
