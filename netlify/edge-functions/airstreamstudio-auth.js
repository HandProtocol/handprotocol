// Password-only cookie gate for /mystichearts/airstreamstudio/* paths.
//
// Set env var in Netlify dashboard:
//   AIRSTREAMSTUDIO_PASSWORD  — required. The shared password (set to "mysticstudio").
//
// Flow:
//   1. GET without valid cookie → renders a styled password form (HAND design).
//   2. POST with correct password → sets HttpOnly cookie + 303 redirects back to the path.
...
//
// Cookie value is SHA-256 of "as-v1:<password>", so the raw password is never stored
// client-side, and rotating the env var instantly invalidates every existing cookie.

const COOKIE_NAME = "airstreamstudio_auth";
const COOKIE_PATH = "/mystichearts/airstreamstudio";
const COOKIE_MAX_AGE = 60 * 60 * 24 * 7; // 7 days
const TOKEN_NAMESPACE = "as-v1";

export default async (request, context) => {
  const expected = Netlify.env.get("AIRSTREAMSTUDIO_PASSWORD");

  if (!expected) {
    return new Response(
      "Airstream Studio page is misconfigured: AIRSTREAMSTUDIO_PASSWORD env var is not set.",
  ...
  // GET without valid cookie → show the gate
...
export const config = {
  path: "/mystichearts/airstreamstudio/*"
};
...
<head>
  ...
  <style>
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    ...
    .gate__eyebrow-dot{
      width:6px;height:6px;border-radius:99px;background:#D97706;
    }
    .gate__title{
...
<body>
  <main class="gate">
    ...
    <div class="gate__card">
      <div class="gate__eyebrow"><span class="gate__eyebrow-dot"></span>Internal report</div>
      <h1 class="gate__title">Mystic Hearts Airstream Studio</h1>
      <p class="gate__lede">This working build report is shared with reviewers only. Enter the access password to continue.</p>
      ...
      <form class="gate__form" method="POST" autocomplete="off" novalidate>
        ...
        <input
          ...
          placeholder="••••••••••"
        >
        <button class="gate__submit" type="submit">Open report</button>
      </form>
      <p class="gate__foot">Don't have access? <a href="mailto:hand@handprotocol.org">Email hand@handprotocol.org</a></p>
      <p class="gate__benediction"><strong>M.A.S.S.</strong> &middot; Mystic Airstream Studio &middot; services Sundays, sound check eternal</p>
    </div>
  </main>
</body>
</html>`;
