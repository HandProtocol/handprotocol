/* Unfuckable With: style-portfolio pill.
   Injected into every style page. Adds a small floating pill:
   "All styles · <Style name> · Pick this". Picks persist in
   localStorage["uw-style-picks"] and can be sent to koH through
   /.netlify/functions/feedback (source "Unfuckable With styles").
...
(() => {
  ...
  send.addEventListener("click", async () => {
    ...
    const who = window.prompt("Your name (optional), so koH knows who is choosing:", "Courtney") ?? "";
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          name: who.trim() || "Courtney (preview)", source: SOURCE, tags: ["🧭 picks", "🎨 style"],
