/* (Sub)culture Audio: style-portfolio helpers. Preview-only; delete at launch.
   1. Contact form: these previews have no backend, so submitting explains
      that instead of sending anything.
   2. Floating pill: "All styles · <Style name> · Pick this". Picks persist in
      localStorage["sca-style-picks"] and can be sent to koH through
      /.netlify/functions/feedback (source "Subculture Audio styles"). */
(() => {
  document.querySelectorAll("[data-preview-form]").forEach((form) => {
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      const status = form.querySelector(".form__status");
      if (status) status.textContent = "Preview only: nothing was sent. On the live site this goes to info@subculture.audio.";
    });
  });
  ...
  const name = (html.dataset.styleName || slug).trim();
  ...
  send.addEventListener("click", async () => {
    ...
    const who = window.prompt("Your name (optional), so koH knows who is choosing:", "") ?? "";
    const note = window.prompt("Anything you want to say about these picks? (optional)", "") ?? "";
    send.disabled = true;
    status.textContent = "Sending…";
    const text = [
      `Style picks: ${list.join(", ")}`,
      `Links: ${list.map((s) => `https://handprotocol.org/project/subculture-audio/${s}/`).join(" ")}`,
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          name: who.trim() || "(Sub)culture Audio (preview)", source: SOURCE, tags: ["🧭 picks", "🎨 style"],
      ...
      try { await navigator.clipboard.writeText(text); status.textContent = "Couldn't send, so your picks were copied to the clipboard instead. Text them to koH."; }
