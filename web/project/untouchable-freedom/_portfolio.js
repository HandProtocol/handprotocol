/* Untouchable Freedom — style-portfolio pill.
   Injected into every style page. Adds a small floating pill:
   "← All styles · <Style name> · ♡ Pick this". Picks persist in
   localStorage["ufc-style-picks"] and can be sent to koH through
   /.netlify/functions/feedback (source "Untouchable Freedom styles").
...
(() => {
  ...
  send.addEventListener("click", async () => {
    ...
    const who = window.prompt("Your name (optional) — so koH knows who is choosing:", "Hannah") ?? "";
    const note = window.prompt("Anything you want to say about these picks? (optional)", "") ?? "";
    send.disabled = true;
    status.textContent = "Sending…";
    const text = [
      `Style picks: ${list.join(", ")}`,
      `Links: ${list.map((s) => `https://handprotocol.org/project/untouchable-freedom/${s}/`).join(" ")}`,
    ...
    try {
      const res = await fetch(ENDPOINT, {
        ...
        body: JSON.stringify({
          ...
          name: who.trim() || "Hannah (preview)", source: SOURCE, tags: ["🧭 picks", "🎨 style"],
