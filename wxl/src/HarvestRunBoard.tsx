...
import { AlertTriangle, CheckCircle2, ClipboardCheck, Clock3, MapPin, Plus, RefreshCw, Route, ShieldCheck, Truck, X } from 'lucide-react'
import {
...
type StopForm = { stop_order: string; stop_type: FoodHarvestStopRecord['stop_type']; rescue_id: string; public_label: string; private_instructions: string; window_start: string; window_end: string; expected_quantity: string; quantity_unit: string }
...
export function HarvestRunBoard({ dbConfigured, canWrite, notify, onAuthRequired }: Props) {
  ...
  const [stopForm, setStopForm] = useState({ stop_order: '1', stop_type: 'pickup' as FoodHarvestStopRecord['stop_type'], rescue_id: '', public_label: '', private_instructions: '', window_start: localFuture(2), window_end: localFuture(3), expected_quantity: '0', quantity_unit: 'lb' })
  ...
  const mutate = async (operation: () => Promise<{ error: { message: string } | null }>, success: string) => {
    setBusy(true)
  ...
  const createRun = async () => {
    ...
    setShowCreate(false); setRunForm({ ...initialRun, starts_at: localFuture(2), ends_at: localFuture(6) })
    await refresh(result.data.id); notify('Harvest run created in planning')
  }
  ...
  const addStop = async () => {
    ...
    setBusy(true)
    const result = await addFoodHarvestRunStop({ ...stopForm, run_id: selected.id, stop_order: Number(stopForm.stop_order), rescue_id: stopForm.stop_type === 'hub' ? undefined : stopForm.rescue_id, window_start: windowStart.toISOString(), window_end: windowEnd.toISOString(), expected_quantity: Number(stopForm.expected_quantity) })
    setBusy(false)
    if (result.error) { notify(result.error.message); return }
    setShowStop(false); await refresh(selected.id); notify('Private run stop added')
  }
  ...
  const recordStop = async () => {
    ...
    const ok = await mutate(() => recordFoodHarvestStop(activeStopId, stopOutcome, Number(observedQuantity), stopNote), stopOutcome === 'incident_hold' ? 'Incident recorded. Run movement stopped.' : 'Stop outcome recorded')
    if (ok) { setActiveStopId(null); setStopNote(''); setObservedQuantity('') }
  }

  if (!dbConfigured) return <section className="panel run-empty"><AlertTriangle size={25} /><h2>Harvest runs need the WXL database.</h2><p>Configure Supabase and apply migration 030 before dispatch testing.</p></section>
  ...
  return <>
    <section className="run-heading"><div><p className="eyebrow"><Route size={14} /> Private reviewed dispatch</p><h2>Harvest runs</h2><p>Coordinators assemble rescue-linked stops, verify exact capability, and assign one approved Contributor. Safety checkpoints remain mandatory.</p></div>{canManage && <button className="add-button" onClick={() => setShowCreate(true)}><Plus size={15} /> Plan run</button>}</section>
    ...
    {showCreate && <RunCreateModal form={runForm} setForm={setRunForm} busy={busy} toggle={toggle} onClose={() => setShowCreate(false)} onSubmit={() => void createRun()} />}
    {showStop && selected && <StopModal form={stopForm} setForm={setStopForm} rescues={rescues} busy={busy} onClose={() => setShowStop(false)} onSubmit={() => void addStop()} />}
    {activeStopId && <div className="modal-backdrop" onClick={() => setActiveStopId(null)}><div className="create-modal stop-outcome-modal" role="dialog" aria-modal="true" aria-labelledby="stop-outcome-title" onClick={(event) => event.stopPropagation()}><div className="modal-title"><h2 id="stop-outcome-title">Record stop outcome</h2><button onClick={() => setActiveStopId(null)} aria-label="Close stop outcome"><X size={18} /></button></div><label>Outcome<select value={stopOutcome} onChange={(event) => setStopOutcome(event.target.value as FoodHarvestStopRecord['status'])}><option value="completed">Completed</option><option value="skipped">Skipped with reason</option><option value="incident_hold">Safety or conduct incident</option></select></label><label>Observed quantity<input type="number" min="0" value={observedQuantity} onChange={(event) => setObservedQuantity(event.target.value)} /></label><label>Outcome evidence and exceptions<textarea rows={3} value={stopNote} onChange={(event) => setStopNote(event.target.value)} /></label><p className="modal-context">Rescue-linked pickup and delivery stops require their safety checkpoint before completion.</p><div className="modal-actions"><button className="cancel-button" onClick={() => setActiveStopId(null)}>Cancel</button><button className="add-button" disabled={busy || !observedQuantity || stopNote.trim().length < 3} onClick={() => void recordStop()}>Record outcome</button></div></div></div>}
  </>
}
