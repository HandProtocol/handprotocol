...
export function HarvestRunBoard({ dbConfigured, canWrite, notify, onAuthRequired }: Props) {
  ...
  const [stopForm, setStopForm] = useState({ stop_order: '1', stop_type: 'pickup' as FoodHarvestStopRecord['stop_type'], rescue_id: '', public_label: '', private_instructions: '', window_start: localFuture(2), window_end: localFuture(3), expected_quantity: '0', quantity_unit: 'lb', compost_pickup_requested: false, expected_compost_quantity: '1', compost_quantity_unit: 'bucket', compost_private_instructions: '' })
  ...
  const recordStop = async () => {
    ...
    const ok = await mutate(() => recordFoodHarvestStop(activeStopId, stopOutcome, Number(observedQuantity), stopNote, compost), stopOutcome === 'incident_hold' ? 'Incident recorded. Run movement stopped.' : compost?.outcome === 'collected' ? 'Delivery and compost return recorded' : 'Stop outcome recorded')
    if (ok) { setActiveStopId(null); setStopNote(''); setObservedQuantity(''); setCollectedCompostQuantity(''); setCompostOutcomeNote(''); setCompostOutcome('collected') }
  }
