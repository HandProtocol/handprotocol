...
export function InventoryBoard({ dbConfigured, canWrite, notify, onAuthRequired }: Props) {
  ...
  const receiveDialogMotion = useDialogMotion(() => setShowReceive(false))
  const [showAllocate, setShowAllocate] = useState(false)
  const allocationDialogMotion = useDialogMotion(() => setShowAllocate(false))
  const [showCondition, setShowCondition] = useState(false)
  const conditionDialogMotion = useDialogMotion(() => setShowCondition(false))
