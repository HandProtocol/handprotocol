...
export function InventoryBoard({ dbConfigured, canWrite, notify, onAuthRequired }: Props) {
  ...
  const mutate = async (operation: () => Promise<{ error: { message: string } | null }>, success: string) => {
    ...
    setActionNote(''); if (selected) await refresh(selected.id); notify(success); return true
  }
  ...
  const receiveLot = async () => {
    ...
    setShowReceive(false); setReceive({ rescue_id: '', storage_location: '', storage_condition: '', minimum_temperature_f: '', maximum_temperature_f: '', use_by_at: localFuture(48), note: '' })
    await refresh(result.data.id); notify('Accepted rescue received into inventory')
  }
  ...
  const reserve = async () => {
    ...
    if (ok) { setShowAllocate(false); setAllocation({ recipient_group: '', quantity: '', pickup_window_start: localFuture(2), pickup_window_end: localFuture(4), private_handoff_instructions: '', note: '' }) }
  }

  const condition = async () => {
    if (!selected) return
    const ok = await mutate(() => recordFoodInventoryCondition({ lot_id: selected.id, temperature_f: temperature ? Number(temperature) : undefined, packaging_ok: packagingOk === 'yes', contamination_concern: contamination === 'yes', storage_control_maintained: storageMaintained === 'yes', note: conditionNote.trim() }), 'Storage condition recorded')
    if (ok) { setShowCondition(false); setPackagingOk(''); setContamination(''); setStorageMaintained(''); setTemperature(''); setConditionNote('') }
  }
...
function SimpleModal({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) { return <div className="modal-backdrop" onClick={onClose}><div className="create-modal inventory-modal" role="dialog" aria-modal="true" aria-labelledby="inventory-modal-title" onClick={(event) => event.stopPropagation()}><div className="modal-title"><h2 id="inventory-modal-title">{title}</h2><button onClick={onClose} aria-label={`Close ${title}`}><X size={18} /></button></div>{children}</div></div> }
