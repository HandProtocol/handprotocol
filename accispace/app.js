...
const SOURCE_PAGE = `https://data.austintexas.gov/resource/${DATASET_ID}`;
...
function addDataLayers() {
  ...
  map.on("mouseleave", "crash-hit-targets", () => { map.getCanvas().style.cursor = "grab"; });
  map.on("click", "crash-hit-targets", (event) => showCrashDetail(event.features[0]));
  map.on("click", (event) => {
...
async function loadCrashData() {
  const fields = [
    "id", "crash_timestamp_ct", "latitude", "longitude", "crash_sev_id", "death_cnt",
...
function showCrashDetail(feature) {
  ...
  document.querySelector("#detail-mode").textContent = p.units_involved || "Not recorded";
  document.querySelector("#detail-source").href = `${SOURCE_PAGE}/${p.id}`;
...
document.querySelector("#toggle-terrain").addEventListener("click", (event) => {
  ...
  event.currentTarget.lastChild.textContent = state.terrain ? " Terrain on" : " Terrain off";
});
...
function setExploreOpen(open) {
  ...
  elements.mobileToggle.querySelector("span").textContent = "Explore data";
}
