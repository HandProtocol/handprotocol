# Capacity Statement

> Why HAND Protocol Foundation is ready to receive funds, deploy them well, and report on them honestly. Used as an attachment to grant applications where funders ask about organizational capacity. Honest about the fact that HAND is small and pre-track-record.

**Status:** Draft v0.1 · May 2026 · Living document

---

## Honest framing

HAND Protocol Foundation is a small, founder-led, pre-incorporation organization preparing to file for 501(c)(3) status in Texas in 2026. This Capacity Statement is honest about that posture. HAND is not a scaled operation with a decade of audited financials. HAND is a focused, early-stage organization with rigorous discovery work, a defensible model, and a deliberate plan to build the operating infrastructure that justifies funder trust.

This document tells you what HAND has now, what HAND is building, and how HAND will use your dollars responsibly during the build.

## What HAND has now (as of May 2026)

### Strategic clarity

- A published gap analysis: the U.S. capacity-building landscape map at handprotocol.org/discovery, the resource-pool vision document, and the research-on-existing-skill-exchange-models document. Together, ~30,000 words of cited research, named peer organizations, and identified market gaps.

- A theory of change with five named links and honest assessment of the evidence for each. Documented at [`../programs/theory-of-change.md`](../programs/theory-of-change.md).

- A three-year strategic plan with priorities, milestones, budget envelopes, and risk-mitigation. Documented at [`../programs/strategic-plan-2026-2028.md`](../programs/strategic-plan-2026-2028.md).

- A logic model linking inputs to impact. Documented at [`../programs/logic-model.md`](../programs/logic-model.md).

### Brand and public presence

- The handprotocol.org website with three connected surfaces (foundation campaign, discovery hub, legacy archive). Live since May 2026.

- A coherent brand: voice (earnest, evidence-based, relational), visual identity, accessibility commitments (WCAG 2.1 AA on all body text and interactive elements).

- An audit-passed implementation. Every page reviewed against design and content quality standards. No vendor-template aesthetics.

- A mailing list with double opt-in (Resend), an active Discord, and a Twitter/X presence.

### Governance documents

- Draft Articles of Incorporation with IRS-mandated language for 501(c)(3) qualification.
- Draft Bylaws.
- Draft of all Form 990 Part VI-disclosed policies (Conflict of Interest, Whistleblower, Document Retention, Executive Compensation).
- Draft operational policies (Financial Management, Gift Acceptance, Fiscal Sponsorship, Grant Management, Code of Ethics, Equal Opportunity and Anti-Harassment, Privacy, Data Sovereignty and AI, Community Standards, Volunteer and Contributor).
- Draft board governance package (Member Agreement, Succession and Replacement, Emergency Succession Plan, Officer Job Descriptions, Composition Matrix, Annual Disclosure Form).
- Draft program documents (Companion Selection and Graduation Protocol).

All published in this repository under [`../`](../).

### Domain expertise

- Founder with: [founder bio summary, to be filled in at adoption, typically 1-2 paragraphs covering: prior work; recognition as practitioner; relevant relationships; specific gap-identification track record]

- Advisory and Contributor relationships across the practitioner, technology, and capacity-building fields.

- Direct knowledge of the populations HAND serves through prior collaboration, lived experience, or close practitioner relationships.

### Funder pipeline

- Patrick J. McGovern Foundation: Letter of Inquiry drafted, alignment with AI for Social Good program identified.
- Texas community foundations: Hogg, St. David's, Austin Community Foundation identified as future eligibility targets.
- Trust-based funders: Kataly, Solidaire, Headwaters identified as aligned philosophical kin.
- AI-aligned funders: Mozilla, Ford, Google.org identified as additional prospects.

### Sovereign Companions program design

- Eight sovereignty principles articulated and binding ([`../../AI-COMPANIONS.md`](../../AI-COMPANIONS.md)).
- Evaluation framework drafted ([`../../AI-EVAL-FRAMEWORK.md`](../../AI-EVAL-FRAMEWORK.md)): six dimensions, three decision gates, participatory review panel.
- Data Sovereignty and AI Policy operationalizing the principles ([`../policies/data-sovereignty-and-ai.md`](../policies/data-sovereignty-and-ai.md)).

## What HAND is building (next 12 months)

### Incorporation and federal exemption

- Texas Form 202 filing (target: Q3 2026).
- IRS Form 1023 filing (target: Q3-Q4 2026, within the 27-month retroactive window).
- IRS Determination Letter expected mid-2027.
- Texas Comptroller exemption (AP-204) immediately after.

### Founding board

- Three initial directors at incorporation.
- Expansion to five by year-end 2026.
- Skills, lived experience, and network priorities documented in the Composition Matrix.

### First Companion cohort

- Three Companion groups, one from each audience segment (healer, impact entrepreneur, grassroots).
- Onboarding through Q4 2026 and Q1 2027.
- Three-year engagement commitment from each side.

### Contributor pool

- 10 active Contributors by year-end 2026.
- Curated, screened, mission-aligned.

### Sovereign Companions POC

- Proof-of-concept agent system, scaffolded on existing AI infrastructure (closed-API for prototyping permitted at this stage).
- First sovereignty drill (revocation, portability simulation) by end of Year 1.
- Methodology documented and published.

### Financial infrastructure

- Banking, accounting system, payroll provider in place.
- Insurance (D&O, general liability, cyber) bound.
- First-year financial reporting (compilation, not audit, given pre-revenue scale).

## How HAND will use your funds responsibly

### Financial controls in place from day one

- **Signing authority limits.** Single-signature ≤ $5,000; dual-signature $5,000–$25,000; dual + board resolution > $25,000.
- **Monthly bank reconciliation** by an independent reconciler.
- **Quarterly Treasurer review** of all accounts.
- **Restricted funds tracked separately** at the account and ledger level.
- **No loans to directors or officers** (prohibited by Bylaws).
- **Rebuttable-presumption procedure** in place for any executive compensation.

### Restricted-funds discipline

- Grants are recorded as temporarily restricted at receipt.
- Expenses are coded to the grant fund as incurred.
- Variance reports comparing budget to actuals run quarterly.
- Material variances (>10% on any category) require funder communication before incurring.
- Unspent funds returned per grant agreement, not retained without funder consent.

### Reporting cadence

- Monthly internal financial statements.
- Quarterly Board financial reports.
- Funder-specific reports per grant agreement.
- Annual Report (public).
- Annual Form 990 filed timely.
- Annual AI Accountability Report (public) for Sovereign Companions program.

### Compliance

- The Compliance Calendar ([`../COMPLIANCE-CALENDAR.md`](../COMPLIANCE-CALENDAR.md)) tracks every filing, renewal, and review.
- The Document Retention Policy applies to all funder-relevant records.
- Document destruction is suspended upon any indication of audit, investigation, or litigation.

### Independent oversight

- Treasurer is an independent director, not the Executive Director.
- Finance Committee reviews monthly statements.
- Audit Committee engages external CPA for compilation in Year 1, review in Years 2-3, audit when triggered by revenue or funder requirement.
- Board approves the budget; Executive Director executes.

## What HAND will share with you

### Standard funder package

- IRS Determination Letter (once received).
- Most recent Form 990 (or 990-N if applicable in early years).
- Compiled, reviewed, or audited financials per year.
- Board roster and conflict-of-interest disclosures.
- Current operating budget.
- Strategic plan and theory of change.
- Logic model.
- Quarterly Sovereign Companions reports.
- Annual Report.

### Voluntary transparency

- All governance documents on the website.
- All policies on the website.
- Theory of change and strategic plan on the website.
- Compensation ratio in the Annual Report.
- Fundraising-cost-to-funds-raised ratio in the Annual Report.

### Communication

- Major milestones communicated proactively.
- Site visits welcomed, with Companion-group consent.
- Convenings attended.
- Honest about setbacks before being asked.

## What HAND will not do with your funds

- Pay for institutional rhetoric or aspirational language that misrepresents the work.
- Optimize the program ratio at the expense of the work that matters.
- Spin negative outcomes as positive.
- Co-mingle restricted and unrestricted funds.
- Engage in lobbying or political campaign activity (per Bylaws Article 4 and IRC §501(c)(3)).
- Use AI funds for non-AI work, or non-AI funds for AI infrastructure HAND has not separately disclosed.
- Train AI models on Companion group data without explicit, signed consent.
- Compromise the eight Sovereign Companions sovereignty principles for funder convenience.

## What's missing right now, honestly

A capacity statement that pretends to have everything is not credible. HAND has not yet:

- Filed for 501(c)(3) status (in progress).
- Received the IRS Determination Letter (expected mid-2027).
- Hired a paid Executive Director (founder is the only operational lead currently).
- Closed a full Year 1 of Companion engagement and produced outcomes data.
- Engaged an external CPA (will engage at incorporation).
- Carried D&O and general liability insurance (will bind before any program activity).
- Run a full rebuttable-presumption procedure (will run before first compensation).

The strategic plan and the funder pipeline are designed to remedy each of these in a defined timeline. The Compliance Calendar tracks the dates.

## Why funder trust is well-placed here

The honest case for funding HAND:

a. **Rigorous discovery work.** The published research demonstrates that HAND has done the homework on the field and the gap.

b. **Defensible model.** Long-term relational accompaniment is hard. The unit economics challenge is real. HAND's design (Sovereign Companions as parallelization, curated Contributor pool, three-year engagement) is a credible response.

c. **Sovereignty posture.** HAND's commitment to group-owned AI, open methodology, and inspectable governance is rare. It is the responsible-AI grantee profile some funders explicitly seek.

d. **Voice integrity.** HAND has retired tools that did not serve the Companions (the pivot from Web3). The same posture applies to anything else that doesn't earn its place.

e. **Transparency from day one.** Governance documents, theory of change, strategic plan, and policies are all public before incorporation. This is the floor, not the ceiling.

f. **Honest about the unknowns.** Every document has an "open questions" section. Funders trust organizations that name their gaps before being asked.

---

*HAND Protocol Foundation. Pre-incorporation. Contact: hand@handprotocol.org. Documents: handprotocol.org/discovery and handprotocol.org/governance.*
