# Funder Pipeline

> The current funder shortlist, prioritized by fit and access, with relationship status, deadlines, and tactical notes. Living document, reviewed quarterly by the Finance Committee.

**Status:** Draft v0.1 · May 2026 · Living document · Reviewed quarterly

---

## How to read this document

The funder pipeline is HAND's master tracker for fundraising. Each entry includes:

- **Funder.** Name and program if relevant.
- **Fit.** Why this funder aligns with HAND.
- **Access path.** RFP, invitation-only, relationship-led, peer panel.
- **Range.** Typical grant size.
- **Relationship status.** Cold, warming, in conversation, applied, awarded, declined.
- **Next action.** What HAND does next, by when.
- **Notes.** Specific intelligence, due diligence, cautions.

The pipeline is **not** a public document. The repository carries the structural template; specific outreach status is maintained in the Executive Director's working copy.

## Section 1. Fit tiers

Funders are ranked by alignment with HAND's posture across three vectors:

a. **Trust-based posture.** Multi-year unrestricted, relational, minimal reporting burden.

b. **Mission alignment.** Capacity-building, accompaniment, grassroots, healers, impact entrepreneurs.

c. **AI sovereignty alignment.** Open-source AI, group-owned data, responsible AI infrastructure.

A funder strong on all three is a **Tier 1** prospect. Strong on two is **Tier 2**. Strong on one is **Tier 3**.

## Section 2. Tier 1 prospects

These funders are the closest philosophical kin to HAND. Most are relationship-led; the path is sustained engagement, not application.

### Patrick J. McGovern Foundation

- **Fit:** AI sovereignty alignment (highest). $73.5M+/year toward AI for Humanity. Programs: AI for Climate, Health Equity, Human Rights, Digital Future. Data Practice Accelerator: up to $125K/year for responsible-data orgs.
- **Access:** Invitation-only; LOI a typical first step where opening exists.
- **Range:** $50K–$500K typical.
- **Status:** LOI drafted ([`../../funding/mcgovern-letter.md`](../../funding/mcgovern-letter.md), $333,223 ask over 18 months).
- **Next action:** Identify program officer; warm introduction via Amnesty's Algorithmic Accountability Lab or other current grantee. Submit LOI when introduction is in hand.
- **Notes:** McGovern wants sovereignty posture, open source, group ownership. HAND's Sovereign Reciprocates program is a strong fit. Donor-advised LOI process; expect 6-12 months from first contact to formal application.
- **Risk:** Highly selective. Plan B is essential.

### Kataly Foundation

- **Fit:** Mission alignment (highest); trust-based posture (highest). Spend-down foundation, Restorative Economies program directly aligned.
- **Access:** Invitation-only. **No unsolicited applications.** Path is via current grantee or staff orbit.
- **Range:** $100K–$2M+ multi-year unrestricted.
- **Status:** Cold.
- **Next action:** Identify current Kataly grantees in HAND's network; build authentic relationships before any ask.
- **Notes:** The single most aligned funder philosophically. The patience-and-relationships requirement is real; don't rush.
- **Risk:** Low yield without warm pathway.

### Solidaire Network

- **Fit:** Mission alignment (high). Member-funded donor network; grants flow from member-led funds.
- **Access:** Relationship-led, similar to Kataly.
- **Range:** Varies by member fund.
- **Status:** Cold.
- **Next action:** Participate in Solidaire convenings as observer; meet member-fund leaders.
- **Notes:** Like Kataly, this is a multi-year relationship build.

### MacKenzie Scott / Yield Giving

- **Fit:** Mission alignment (high); trust-based posture (highest).
- **Access:** Open Call rounds via Lever for Change; peer panel review. Direct selection by Yield Giving team based on field intelligence.
- **Range:** $1M–$10M+ multi-year unrestricted.
- **Status:** Not yet eligible (need IRS Determination + operating history).
- **Next action:** Maintain pipeline awareness; apply to next Open Call after Year 2 financials are available.
- **Notes:** Highly competitive. Strategic plan, audited financials, business plan all required. Audited financials are HAND's gap until Year 2-3.
- **Risk:** Long shot but high upside if selected.

## Section 3. Tier 2 prospects

Strong on two of three alignment vectors. Often more accessible than Tier 1.

### Hogg Foundation for Mental Health (Texas)

- **Fit:** Mission alignment (high) for healer pillar. Texas-domiciled funder, ideal for Texas-domiciled HAND.
- **Access:** RFP-driven only. No unsolicited. Fluxx grants management system.
- **Range:** $50K–$300K typical.
- **Status:** Cold; monitoring RFPs.
- **Next action:** Subscribe to Hogg RFP announcements; identify program officers; attend Hogg-hosted convenings.
- **Notes:** Texas-based 501(c)(3) generally required; HAND eligible once IRS Determination arrives. Strong fit if Hogg releases an accompaniment-themed RFP.
- **Risk:** RFP-dependent; cannot proactively shape timing.

### St. David's Foundation (Texas)

- **Fit:** Mission alignment (moderate-high) for healer pillar; Central Texas geographic fit.
- **Access:** Invitation-only; program officers will discuss eligibility.
- **Range:** $50K–$500K+ typical, multi-year possible.
- **Status:** Cold; identify program officers.
- **Next action:** Request informational meeting with relevant program officer once filing is in.
- **Notes:** ~$100M/year grantmaker; strong commitment to mental health and culturally-responsive care.

### Mozilla Foundation (Trustworthy AI / Responsible Computing)

- **Fit:** AI sovereignty alignment (highest); some mission overlap.
- **Access:** RFP-driven; fellowships and project grants.
- **Range:** $25K–$200K typical.
- **Status:** Cold; monitoring RFPs.
- **Next action:** Apply to next Responsible Computing Fellowship cycle; submit project proposal in next themed call.
- **Notes:** Open-weights and Reciprocate-sovereignty posture is exactly Mozilla's brief.

### Ford Foundation (Tech & Society)

- **Fit:** AI sovereignty alignment (high); mission alignment moderate.
- **Access:** Invitation-only. Relationship-led.
- **Range:** $100K–$1M+.
- **Status:** Cold.
- **Next action:** Build presence in Mozilla / Ford convenings; identify program officer.

### Google.org AI for Social Good

- **Fit:** AI sovereignty alignment (moderate); historically funds capacity-building tooling.
- **Access:** RFP cycles; nonprofit credit programs.
- **Range:** $100K–$1M.
- **Status:** Monitoring; apply for nonprofit credits in parallel.
- **Notes:** Watch for tension between Google's general-AI-platform vision and HAND's open-source per-group sovereignty posture.

### Anthropic + OpenAI (nonprofit credit programs)

- **Fit:** Practical alignment for Sovereign Reciprocates prototype phase.
- **Access:** Application-based credit programs.
- **Range:** API credits, often $5K–$20K equivalent.
- **Status:** Apply once 501(c)(3) status is in place or via fiscal sponsor.
- **Next action:** Submit applications when EIN is issued.
- **Notes:** Useful for burst capacity during prototyping. Not a substitute for the long-term open-source stack.

### Hemera Foundation

- **Fit:** Mission alignment (high) for healer pillar; contemplative-practice support.
- **Access:** Mostly invitation; some RFP windows.
- **Range:** $25K–$150K.
- **Status:** Cold.
- **Next action:** Research current portfolio; identify warm pathway.

## Section 4. Tier 3 and aligned prospects

Reasonable alignment on one vector; lower priority unless an opportunity opens.

### Austin Community Foundation

- **Fit:** Geographic fit (Central Texas).
- **Access:** Open application portal.
- **Range:** $1K–$75K, average $25K. Recommends ≤30% of annual budget.
- **Status:** Not yet eligible (need $50K min budget + 2 years operating history).
- **Eligibility date:** ~2028. Apply then.
- **Notes:** Strong relationship-building opportunity in interim through ACF events and the Greater Austin philanthropy community.

### Headwaters Foundation for Justice (Minneapolis)

- **Fit:** Mission alignment (highest); philosophical kin.
- **Access:** Geographically limited to Minnesota. Not a direct funder for Texas-domiciled HAND.
- **Status:** Reference model, not direct funder.
- **Notes:** Useful as a model and as a connector to aligned Texas funders.

### NDN Collective

- **Fit:** Mission alignment for indigenous-led grassroots Reciprocate groups; data sovereignty aligned (CARE principles).
- **Access:** Invitation/relationship.
- **Status:** Cold.
- **Notes:** Pathway: if HAND has indigenous-led Reciprocate groups, NDN may be a partner more than a funder.

### Hugging Face Community Grants

- **Fit:** Open-source AI legitimacy signal.
- **Access:** Application-based; small grants.
- **Range:** $1K–$10K.
- **Status:** Apply once Sovereign Reciprocates has a published methodology.
- **Notes:** Low dollar, high legitimacy. Good for the open-source release moment in Year 2-3.

### Echoing Green, Halcyon, Roddenberry Foundation, RSF Social Finance

- **Fit:** Mission alignment moderate for impact-entrepreneur pillar.
- **Access:** Various.
- **Status:** Defer until program shape includes formal entrepreneur pipeline.

## Section 5. Individual donor pipeline

The angel-number tier ladder structures individual giving:

| Tier | Amount | Role |
|---|---|---|
| **Foundation** | $222,222 / $77,444 / $22,777 | Lead anchors for filing raise |
| **Sovereign Reciprocates** | $333,223 / $99,777 / $11,113 | AI workstream anchors |
| **Contributor** | $222 to $7,777+ | Broad-base support |

Major-donor tracker maintained separately by the Executive Director. Per [`../policies/gift-acceptance.md`](../policies/gift-acceptance.md), individual major gifts above 2% of expected 5-year revenue are reviewed for public-support-test implications.

## Section 6. DAF and workplace giving

Worth setting up infrastructure even before any inflow:

| Platform | Status |
|---|---|
| **Benevity** | Apply once IRS Determination is in. Workplace giving and matching gifts portal. |
| **YourCause** | Same. |
| **Fidelity Charitable, Schwab Charitable, Vanguard Charitable** (DAF sponsors) | Receive DAF grants once IRS Determination is in. No application required from HAND. |
| **The Giving Block** | Crypto donations infrastructure once needed. |
| **Network for Good** | Backup donation infrastructure. |

## Section 7. Pipeline metrics

Tracked quarterly:

- **Pipeline conversion rate** (LOI to award, application to award).
- **Time-in-pipeline** by funder and stage.
- **Total committed** vs. **total in pipeline**.
- **Funder diversification ratio** (top funder % of total revenue).
- **Application time investment** (hours per application; ROI).

## Section 8. What HAND does not pursue

Honesty about what's outside the pipeline:

a. **Federal grants.** Compliance burden disproportionate to capacity at HAND's stage. Revisit at Year 4+.

b. **Corporate sponsorships** where mission alignment is weak. HAND prefers mission-aligned small business and B-Corp partnerships if they emerge organically.

c. **Crypto-specific grant programs** (Gitcoin, Optimism, etc.). HAND's pivot away from crypto-first positioning means these are not aligned. Crypto donations continue at the small inflow level via the donate-crypto page, but HAND does not pursue crypto-native funding rounds.

d. **AI vendor sponsorships** that compromise sovereignty principles. No "powered by [vendor]" relationships; vendor credits are operational tools, not partnership endorsements.

e. **Anything that would require modifying the eight Sovereign Reciprocates principles.** Funder pressure to do so is itself a signal of misalignment.

## Section 9. Relationship cultivation discipline

For each Tier 1 and Tier 2 funder:

a. **Touchpoint cadence** at least quarterly. Updates, invitations, useful content.

b. **No begging.** Reach out with substance (a published piece, an invitation, a question), not with an ask.

c. **Honor "no."** A declined application is feedback. Don't disappear; maintain the relationship.

d. **Track in a CRM.** Even a Google Sheet for now; graduate to HubSpot Nonprofit, Salesforce NPSP, or Bloomerang once volume warrants.

e. **The Executive Director (or founder pre-ED) owns Tier 1 relationships.** Don't delegate the depth-building to a development hire prematurely.

## Open questions

- **Specific outreach plan for McGovern.** Pre-LOI introduction pathway not yet identified. Active research in May 2026.
- **CRM choice.** Defer to revenue justifying the investment. Google Sheets sufficient through Year 1.
- **Capital campaign.** Major-gift capital campaign for the $222,222 filing raise is being run organically. Whether to formalize for the post-Determination phase (a "Year 2 stabilization campaign") is open.
- **Earned revenue.** Some Reciprocates may have budget to pay for premium support. Whether to structure earned revenue as a real funder category or keep it case-by-case is open. Recommend keeping case-by-case until Year 3.
- **Fundraising consultant.** Whether to engage one (Spectrum Nonprofit Services, Marts & Lundy, Compass Point) at any point. Likely not until $300K+ ask in pipeline.

---

*Reviewed quarterly by the Finance Committee. Outreach status tracked in working copy maintained by the Executive Director.*
